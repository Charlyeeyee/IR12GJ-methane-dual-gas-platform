...
This script processes prepared calibration data from an Excel workbook. The required input columns are methane concentration in `% v/v` and averaged normalised absorbance (`NA_avg`). The script fits the modified Beer–Lambert response model using a fixed experimentally determined SPAN value, then estimates the linearisation coefficients `a` and `n`.
...
- Generate a side-by-side figure showing the calibration fit and residuals.
